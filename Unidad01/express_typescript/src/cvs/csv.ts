...
export const buscarUsuario = async (idUsuario: string): Promise<any> => {
    return new Promise(async (resolve, rejects) => {
        try {
            const usuarios = await cargarTodoslosUsuarios();
            const resultados = usuarios.filter((usuario: Usuario) => usuario.id === idUsuario);
            resolve(resultados);
        } catch (error) {
            rejects((error as Error).message)
        }
...
const agregarDatosCSV = async (datosCsv: Usuario): Promise<string> => {
...
export const guardarUsuario = async (usuario: Usuario): Promise<string> => {
    usuario.id = uuidv4();
    return await agregarDatosCSV(usuario);
}
